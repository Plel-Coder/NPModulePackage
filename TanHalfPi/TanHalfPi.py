...
#TanHalfPi([x,y])
#turn (x * (10 ** y)) to type "TanHalfPi"
#a.getnum()
#try turn a to type "Decimal"
#If fail, raise TooBigNumberError error, and return with type "TanHalfPi"
#a.getex()
#turn a to (x * (10 ** y))

#TODO:
#Please import with alice "thp"
...
class TanHalfPi(object):
    def __init__(self,array):
        if type(array[0])!=Decimal:                             #Turn array to "Decimal"
            array[0]=Decimal(array[0])
        if len(array)==1:                                       #Just number here
            if array[0]<0:
                self.negative=True
                array[0]=abs(array[0])
            else:
    ...
    def getnum(self):                                           #Decimal형태로 출력(실패시 경고 출력하고 TanHalfPi상태로 출력)
        try:
            ...
            raise TooBigNumberError("Use getex() or use operator")
    ...
    def __eq__(self,a):                                         #출력 함수(__eq__를 쓰면 함수없이도 같은지 확인 가능)
        return (self.number==a.number) and (self.index==a.ind